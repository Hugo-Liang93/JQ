...
def get_stock_st(stocks_list, start_date, end_date, df=True, count=None):
    get_extras('is_st', stocks_list, start_date=start_date, end_date=end_date, df=df, count=count)


# value_type: acc_net_value 基金累计净值,unit_net_value 基金单位净值,adj_net_value 场外基金的复权净值
def get_fund_net_value_by_type(value_type, fund_list, start_date, end_date, df=True, count=None):
    get_extras(value_type, fund_list, start_date=start_date, end_date=end_date, df=df, count=count)


# value_type:futures_sett_price 期货结算价,futures_positions 期货持仓量
def get_futures_value_by_type(value_type, fund_list,
                              start_date, end_date, df=True, count=None):
    get_extras(value_type, fund_list,
